...
sucia=database.load_data_sucia()
...
st.markdown("<p style='font-family: Century Gothic;font-size: 15px; text-align: justified'>Después del proceso de limpieza, la base de datos quedó con <b>{:,}</b> registros de vivienda en venta en el municipio de Mérida, Yucatán.</p>".format(df.shape[0]), unsafe_allow_html=True)
...
HeatMap(data=agg_data[['lat', 'lon', 'counts']], radius=8, max_zoom=14,name="Viviendas",overlay=True, control=True,show=True,
...
fig.add_annotation(x=mean_price, y=max_y, text=f"Mean: {mean_price:.2f} million", showarrow=True, arrowhead=1, yshift=10)
